...
std::list<std::string>
calculateHexUnicodeNumber(std::string funcationUserString) {
    ...
    // process
    for (char inputWord : funcationUserString) {
        std::string charToString(1, inputWord);
        if (dictionary.count(charToString) > 0) {
            answerList.push_back(dictionary[charToString]);
        }
    }
...
int main() {
    // This Program call functions
    std::list<std::string> unicodeName;
